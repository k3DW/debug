...
class TestRunner:
    def __init__(self):
        self.framework_errors : list[str] = []
    ...
    def invoke(self, arg, from_tty):
        try:
            ...
            if frame.name() == "k3::gdb::test_start":
                test_name = read_char_ptr(frame, "name")
                if test_name in runner.test_results:
                    runner.framework_errors.append(f"Test name seen twice: {test_name}")
                ...
                if test_name not in runner.test_results:
                    runner.framework_errors.append(f"Test name missing: {test_name}")
                ...
                runner.framework_errors.append(f"Hit breakpoint in unrecognized stack frame\n\t{frame.name()}\n\t{call_site}")
            ...
            runner.framework_errors.append(f"Unknown error: {e}")

def exit_handler(event):
    if len(runner.framework_errors) != 0:
        print(f"Saw {len(runner.framework_errors)} framework errors:")
        for e in runner.framework_errors:
...
runner.framework_errors.append("Program did not exit, therefore something went wrong")
exit_handler(None)
